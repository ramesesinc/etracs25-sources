...
sudo docker build -t ramesesinc/etracs25-services:255.01.003 .
cd $RUN_DIR/city
sudo docker rmi -f ramesesinc/etracs25-server-city:255.01.003
sudo docker build -t ramesesinc/etracs25-server-city:255.01.003 .
cd $RUN_DIR/municipality
sudo docker rmi -f ramesesinc/etracs25-server-municipality:255.01.003
sudo docker build -t ramesesinc/etracs25-server-municipality:255.01.003 .
cd $RUN_DIR/province
sudo docker rmi -f ramesesinc/etracs25-server-province:255.01.003
sudo docker build -t ramesesinc/etracs25-server-province:255.01.003 .
cd ..
echo ""
